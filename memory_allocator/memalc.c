...
*/
struct mem_place find_place(void * ptr)
{
    struct mem_place mp;
    ...
    if(fst_bl_ptr != NULL && snd_fbl_ptr != NULL && fst_bl_ptr->next_bl == snd_fbl_ptr)
    {
        if(fst_free == 0)
        {
            ...
            set_block_fds(fst_bl_ptr, -(absval(fst_bl_ptr->length)+32+absval(snd_fbl_ptr->length)),
                fst_bl_ptr->prev_bl, snd_fbl_ptr->next_bl, snd_fbl_ptr->prev_fbl,
                snd_fbl_ptr->next_fbl);
...
void free_fcn(void * ptr)
{
    ...
    struct mem_place mp = find_place(ptr);
    struct arena * ar_ptr = mp.ap;
    struct block * bl_ptr = mp.bp;
    ...
    if(free_space > 2*TRH)
    {
        ...
        while(del_ptr != NULL)
        {
            if(del_ptr->length < 0 || (del_ptr->freeblock_list != NULL
                                       && absval(del_ptr->freeblock_list->length) == TRH-64))
            {
