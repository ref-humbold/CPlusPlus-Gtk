...
void SocketController::echo_request(const std::string & addr, uint16_t id, uint16_t seq, int ttl)
{
    for(int i = 0; i < 3; ++i)
    {
...
std::tuple<std::set<std::string>, int> SocketController::echo_reply(uint16_t id, uint16_t seq)
{
...
std::string SocketController::recv_echo(uint16_t id, uint16_t seq)
{
