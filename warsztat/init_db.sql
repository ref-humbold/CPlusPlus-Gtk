...
CREATE TABLE cars (model text PRIMARY KEY,
                  marka text NOT NULL,
                  typ auto_type,
                  waga double precision CHECK(waga > 0));

CREATE TABLE uslugi (nazwa text PRIMARY KEY,
                    cena price_type);

CREATE TABLE zlecenia (id serial PRIMARY KEY,
                      data_zlec timestamp DEFAULT now(),
                      data_real timestamp DEFAULT NULL,
                      nr_rej registration_type,
                      faktura boolean DEFAULT FALSE,
                      koszt price_type,
                      kli_id integer,
                      sam_model text);

CREATE TABLE carparts (id serial PRIMARY KEY,
                      producent text,
                      ilosc integer CHECK(ilosc >= 0),
                      typ text,
                      uniw boolean DEFAULT TRUE);

CREATE TABLE zamowienia (id serial PRIMARY KEY,
                        data_zamow timestamp DEFAULT now(),
                        data_real timestamp DEFAULT NULL,
                        firma text,
                        ilosc integer CHECK(ilosc > 0),
                        cena price_type,
...
CREATE OR REPLACE RULE czesam_rule AS
    ON INSERT TO czesam
    DO ALSO UPDATE carparts
            SET uniw = FALSE
            WHERE carparts.id = NEW.cze_id;

CREATE OR REPLACE FUNCTION zleusl_trg_func() RETURNS TRIGGER AS $f$
BEGIN
    WITH tab AS (SELECT zlecenia.id, sum(uslugi.cena)
                FROM zlecenia
                    JOIN zleusl ON zleusl.zle_id = zlecenia.id
                    JOIN uslugi ON zleusl.usl_nazwa = uslugi.nazwa
                GROUP BY zlecenia.id
                HAVING zlecenia.id = NEW.zle_id)
    UPDATE zlecenia SET koszt = tab.sum
    FROM tab
    WHERE zlecenia.id = tab.id;
